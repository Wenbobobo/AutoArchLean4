...
2. [Phase 6+ Roadmap](docs/roadmap/phase6-autonomous-workspace.md)
3. [Continuous Execution Playbook](docs/roadmap/continuous-execution.md)
4. [环境安装](docs/onboarding/environment.md)
5. [Lean 心智模型](docs/onboarding/lean-mental-model.md)
6. [Archon 原理](docs/onboarding/archon-principles.md)
7. [第一次运行](docs/onboarding/first-run.md)
8. [Operator Console Guide](docs/onboarding/operator-console.md)
9. [常见问题](docs/onboarding/troubleshooting.md)
...
下面这组命令可以直接复制。
你只需要先把 `PROJECT_PATH` 和 `ARCHON_PATH` 改成自己的绝对路径。
...
新开一个终端，启动无人值守 workspace daemon：
...
可选：再开一个终端，手动补一个 worker fleet，方便更快观察 queue / worker 行为：
...
如果你只是执行上面的 `workspace init + dashboard serve + workspace daemon run`，
默认跑的是 workspace 自治 session，而不是 benchmark。

- 默认队列任务类型是 `session_quantum`
- 默认 workflow 是 `adaptive_loop`
- 默认运行模式是 `dry_run = true`
- 默认 iteration 上限是 `10`
- 一个全新项目第一轮通常会先进入 `plan`
- 第一轮常见 reason 是 `bootstrap_first_iteration`

只有你显式执行 `benchmark run` 或 `queue enqueue-benchmark`，
系统才会开始跑 `benchmark_project` 类型任务。
...
uv run archonlab workspace init --project-path /path/to/lean-project --archon-path /path/to/Archon --config-path workspace.toml
uv run archonlab workspace status --config workspace.toml
uv run archonlab workspace start-session --config workspace.toml --project-id demo
uv run archonlab workspace run-project --config workspace.toml --project-id demo --max-iterations 3
uv run archonlab workspace enqueue --config workspace.toml
uv run archonlab workspace resume --config workspace.toml
uv run archonlab workspace daemon run --config workspace.toml
uv run archonlab workspace daemon status --config workspace.toml
uv run archonlab run start --config archonlab.toml --dry-run
uv run archonlab run start --config archonlab.toml --execute
uv run archonlab run loop --config archonlab.toml --max-iterations 3
...
uv run archonlab benchmark runs --manifest benchmarks/smoke.example.toml
uv run archonlab benchmark run-detail --manifest benchmarks/smoke.example.toml --run-id <id>
