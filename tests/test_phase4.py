...
def test_supervisor_policy_distinguishes_healthy_and_stuck_runs(tmp_path: Path) -> None:
    ...
    assert blocked_decision.action is SupervisorAction.CONTINUE
    assert blocked_decision.reason is SupervisorReason.HEALTHY
...
def test_supervisor_policy_treats_pure_sorry_blocked_ratio_as_healthy(
    tmp_path: Path,
) -> None:
    from archonlab.supervisor import decide_supervisor_action
    from archonlab.task_graph import build_task_graph
    ...
    assert decision.reason is SupervisorReason.HEALTHY
...
def test_supervisor_policy_treats_analyzer_fallback_as_infra_issue(tmp_path: Path) -> None:
    ...
    decision = decide_supervisor_action(
        snapshot=_snapshot_for_graph(
            ...
            update={
                ...
                "analysis_fallback_reason": "sidecar unavailable",
            }
        ),
        task_graph=graph,
        recent_events=[EventRecord(run_id="run-1", kind="run.started", project_id="demo")],
    ...
    assert decision.action is SupervisorAction.INVESTIGATE_INFRA
    assert decision.reason is SupervisorReason.ANALYZER_DEGRADED
    assert decision.evidence["analysis_backend"] == "regex"
    assert decision.evidence["analysis_fallback_reason"] == "sidecar unavailable"
